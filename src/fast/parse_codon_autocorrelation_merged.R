...
#####  1. READ DATA
data<-read.table("INPUTFILE",header=T) # file must be "***.merged"
head(data)
ala_data<-data[1:16,]
arg_data<-data[17:52,]
gly_data<-data[53:68,]
...
sink("INPUTFILE_effect_matrix.txt",append=FALSE, split=FALSE)
ala_effect_matrix
arg_effect_matrix
gly_effect_matrix
ile_effect_matrix
...
INPUTFILE<-c(ala_effect_vector, arg_effect_vector, gly_effect_vector, ile_effect_vector, leu_effect_vector, ser_effect_vector, pro_effect_vector, thr_effect_vector, val_effect_vector)  # all_all_vector

write.table(data.frame(INPUTFILE), "INPUTFILE_effect_vector.txt", row.names=F)
...
INPUTFILE<-c(ala_rscpu_vector, arg_rscpu_vector, gly_rscpu_vector, ile_rscpu_vector, leu_rscpu_vector, ser_rscpu_vector, pro_rscpu_vector, thr_rscpu_vector, val_rscpu_vector)  # all_all_vector

write.table(data.frame(INPUTFILE), "INPUTFILE_rscpu_vector.txt", row.names=F)
...
write.table(data.frame(INPUTFILE), "INPUTFILE_rscpu_vector_way2.txt", row.names=F)
