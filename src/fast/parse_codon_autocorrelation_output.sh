...
# Merge files
echo -e "AA\tCodon1\tCodon2\tPaired_total\tTotal_codon1\tTotal_codon2" > $name.codon_counts.merged
cat $paired_codons_file | while read line; do
	codon1=`echo $line | awk {'print $2'}`
	total_codon1=`cat $total_codons_file | grep $codon1 | awk {'print $3'}`
	codon2=`echo $line | awk {'print $3'}`
	total_codon2=`cat $total_codons_file | grep $codon2 | awk {'print $3'}`
